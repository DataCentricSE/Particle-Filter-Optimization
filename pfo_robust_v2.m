function [xopt fxopt M XX XXtr MMM w]=pfo(objective,Ns,boundary,LB,UB,resampling_strategy,fi_exp,tol,uk_noise,weightrate,vars,vars_mean,vars_sigma)
global ment2 ment6 bbb whub_whlb
global G_st G_to G_be G_H2 G_meth G_eth G_steam1 G_steam2
global E_i A_i  unc_x
...
%                 end
         ...
         for j=1:length(vars) %robust
            %assignin('base',vars{j},vars0(j)); %sample the uncertain parameters
%             vvvv= genvarname(vars{j});
             eval([vars{j} ' = (rand()*2-1)*vars_sigma(j)/100*vars_mean(j)+vars_mean(j);']); %uniform
%              eval([vars{j} ' = normrnd(vars_mean(j),vars_sigma(j)/100*vars_mean(j));']); %normal
         end
%          fx{k}(i)=objective(X{k}(i,:)); %KE: zhou2011 szerint Hx, nálam fx a jelölés
%          fx{k}(i)=objective(normrnd(X{k}(i,:),(LB+UB)/2.*unc_x/100));
%          %KE: zhou2011 szerint Hx, nálam fx a jelölés; %KENEW:
%          uncertainty on x!; normal
%          fx{k}(i)=objective((rand(1,length(LB))*2-1).*unc_x/100.*(LB+UB)'/2+X{k}(i,:)); %KE: zhou2011 szerint Hx, nálam fx a jelölés; %KENEW: uncertainty on x!; uniform
         fx{k}(i)=objective((rand(1,length(LB))*2-1).*unc_x/100.*(UB-LB)'/2+X{k}(i,:)); %KE: zhou2011 szerint Hx, nálam fx a jelölés; %KENEW: uncertainty on x!; uniform
...
%     y(k)=max(fx{k})-vk(max(fx{k}),min(fx{k})); %old/original
    y(k)=max(fx{k})-(max(fx{k})-min(fx{k}))*0.3; %KENEW; upper 30% is kept

   

    if k>1 %KENEW (not neccessary if y is fixed...)
    if y(k)<y(k-1)
        y(k)=y(k-1);
    end
    end
...
%          if y(k)<=y(k-30) %KEJAV
        ...
        if u_switch==0
             [~,ddd]=knnsearch(X{k},X{k},'k',round(Ns/20)); %az Ns/20-adik legközelebb szomszéd legyen a tol-on belül...
             if mean(ddd(:,end))<sqrt(sum(tol(UB-LB~=0).*tol(UB-LB~=0))) %Pitagorasz
%             if (max(X{k}(:,1))-min(X{k}(:,1)))/tol(1)*(max(X{k}(:,3))-min(X{k}(:,3)))/tol(3)<Ns %KENEW
%             if prod((max(X{k}(:,UB-LB~=0))-min(X{k}(:,UB-LB~=0)))./tol(UB-LB~=0))<Ns %KENEW csak x length independetn
             u_switch=1;
             sw=k
             end
        end
        ...
        [uu1]=unique(X{k}(:,1)); %original (before 05.31)

%         [uu1]=uniquetol(X{k},max(UB-LB)/100,'ByRows',true)
%        stop(k)=any([all([max(X{k}(:,1))-min(X{k}(:,1))<=tol(1),max(X{k}(:,2))-min(X{k}(:,2))<=tol(2),max(X{k}(:,3))-min(X{k}(:,3))<=tol(3),uk_noise(1)*k^uk_noise(2)>0]),max(fx{k})-min(fx{k})<0.2, max(max(histc(X{k},uu1)))>Ns*0.8]);%*ones(1,Nx);
       stop(k)=any([all([max(X{k})-min(X{k})<=tol,uk_noise(1)*k^uk_noise(2)>0]), max(max(histc(X{k}(:,1),uu1)))>Ns*0.8,max(fx{k})-min(fx{k})<(max(fx{1})-min(fx{1}))/100000]);% %*ones(1,Nx); %0.2 --> (max(fx{1})-min(fx{1}))/100
