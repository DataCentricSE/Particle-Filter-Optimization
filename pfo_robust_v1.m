...
%     y(k)=max(fx{k})-vk(max(fx{k}),min(fx{k})); %old/original
    y(k)=max(fx{k})-(max(fx{k})-min(fx{k}))*0.4; %KENEW; upper 30% is kept
    ...
    %Resampling
     ...
     if k>30
%          if y(k)<=y(k-30) %KEJAV
         if (max(X{k}(:,1))-min(X{k}(:,1)))/tol(1)*(max(X{k}(:,2))-min(X{k}(:,2)))/tol(2)<Ns %KENEW
             u_switch=1;
         end
         %M(k)=mean(X{k});
        M(k)=sum(fx{k}.*w{k})/sum(w{k}); %k>2!!!
        % stop(k)=any([all([abs(M(k)-M(k-1))<tol,uk_noise(1)*k^uk_noise(2)>0]),max(fx{k})-min(fx{k})<1]);%*ones(1,Nx);
       stop(k)=any([all([max(X{k}(:,1))-min(X{k}(:,1))<tol(1),max(X{k}(:,2))-min(X{k}(:,2))<tol(2),max(X{k}(:,3))-min(X{k}(:,3))<tol(3),uk_noise(1)*k^uk_noise(2)>0]),max(fx{k})-min(fx{k})<0.2]);%*ones(1,Nx);
